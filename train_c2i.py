...
def main(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for data_iter_step, (x, y) in enumerate(loader):
            ...
            adjust_learning_rate(optimizer, data_iter_step / len(loader) + epoch, args)
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype):
                _, loss = model(z_indices, c_indices, targets=z_indices)
            ...
            if train_steps % args.log_every == 0:
                ...
                logger.info(f"(step={train_steps:07d}) Train Loss: {avg_loss:.4f}, LR: {lr:.6f}, Train Steps/Sec: {steps_per_sec:.2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cloud-save-path", type=str, default="./checkpoints", help='please specify a cloud disk path, if not, local path')
